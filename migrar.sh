	...
	if [ $HASDB = "true" ]
		then
			...
			case $PARAM in
				...
				read DBPASS;;
				8) echo "Insira o servidor de banco de origem:"
				read DBORI;;
				*) echo "Opção inválida"
				   func_change_param;;
			esac
		else
			# Caso não tenha banco, fará teste sem os parame�s do banco
...
if test $HASDB = 'true' 
then	
	ssh -l root $SERVER "(echo 3; echo ${SITE}; echo ${ESTRUTURA}; echo '${DBNAME}' ; echo '${DBUSER}' ; echo '${DBPASS}') | /root/bin/cria_dominio_bd.sh"
else
	ssh -l root $SERVER "(echo 1; echo ${SITE}; echo ${ESTRUTURA}) | /root/bin/cria_dominio_bd.sh"
fi

echo ""
echo "Apagando index.html padrao na estrutura '${SITE}'"

ssh -l root $SERVER "rm /home/wvirt/'${SITE}'/public_html/index.html"
echo "#############################################################"
#clear
...
rsync -hraz /home/wvirt/$SITE/public_html $SERVER:/home/wvirt/$SITE/
rsync -hraz /home/wvirt/$SITE/logs/*  $SERVER:/home/wvirt/$SITE/var/log/
rsync -hraz /home/wvirt/$SITE/stats $SERVER:/home/wvirt/$SITE/var/
rsync -hraz /home/wvirt/$SITE/data  $SERVER:/home/wvirt/$SITE/var/
...
echo "#############################################################"
echo " "
echo " Alterando as permissões da pasta public_htm para pasta stats para o grupo $SITE "

ssh -l root $SERVER "chown -R $SITE:$SITE /home/wvirt/$SITE/public_html"
ssh -l root $SERVER "chown -R $SITE:$SITE /home/wvirt/$SITE/var/stats"
...
ssh -l  root $SERVER "ln -s /home/wvirt/$SITE/var/log/access.2017.11.log /home/wvirt/$SITE/var/log/access_log"
...
then
	...
	echo "Realizando dump do banco $DBNAME & enviando o dump do servidor de origem para a STEPUP..."

	CMDDUMP="/root/bin/migrar_2.sh '${DBDEST}' '${DBNAME}'"

	if [ "$DBORI" = "localhost" ]
		then
			test -d /home/dump-bancos && echo "Iniciando o dump..." || mkdir /home/dump-bancos
			echo "Iniciando o dump do banco ${DBNAME}.sql"
	       		mysqldump ${DBNAME} > /home/dump-bancos/${DBNAME}.sql
			echo "Enviando o banco '${DBNAME}.sql' para o servidor de destino"
			rsync -hrazv /home/dump-bancos/${DBNAME}.sql root@$DBDEST:/home/Bancos-Panther/$DBNAME.sql

			# Restaurando o banco no servidor de banco de destino
			echo -e "\n
				----------------------------------------------------------- \n \
				| Restaurando banco '${DBNAME}'.sql No servidor de destino... |\n \
				------------------------------------------------------------"
			ssh root@$DBDEST "mysql '${DBNAME}' < /home/Bancos-Panther/${DBNAME}.sql"

		else 
			# Executa parametros para bancos remotos - Firehawk
			#ssh -l root $DBORI "$CMDDUMP"
			ssh -l root $DBORI "test -d /home/dump-bancos && echo 'Iniciando o dump...' || mkdir /home/dump-bancos; \
					echo 'Iniciando o dump do banco $DBNAME.sql'; \
	       				mysqldump '$DBNAME' > /home/dump-bancos/'$DBNAME'.sql; \
					echo 'Enviando o banco '$DBNAME.sql' para o servidor de destino'; \
					rsync -hrazv /home/dump-bancos/'$DBNAME'.sql root@$DBDEST:/home/Bancos-Panther/$DBNAME.sql"
